...
class RiskProfile:
    ...
    questionnaire_reliability: float = 0.5  # 不改UI下的隱性信號品質


class PsychologicalFitGuard:
    """反對觀點防線：短問卷只提供粗糙訊號，避免過度自信推論。"""

    MIN_RECOMMENDED_ITEMS = 10

    @staticmethod
    def estimate_reliability(responses: Dict[str, float], weights: Dict[str, float]) -> float:
        """估計問卷可靠度：題數不足、權重過度集中時降低信任。"""
        n_items = max(len(responses), 1)
        item_factor = np.clip(n_items / PsychologicalFitGuard.MIN_RECOMMENDED_ITEMS, 0.0, 1.0)

        w = np.array(list(weights.values()), dtype=float)
        w = w / w.sum() if w.sum() > 0 else np.ones_like(w) / len(w)
        concentration = np.sum(w ** 2)
        effective_items = 1 / concentration if concentration > 0 else len(w)
        concentration_factor = np.clip(effective_items / len(w), 0.0, 1.0)

        r = np.array(list(responses.values()), dtype=float)
        dispersion = np.std(r)
        # 不是追求越一致越好：完全一致或完全極端都意味辨識力有限
        discrimination_factor = np.clip(1 - abs(dispersion - 0.28) / 0.28, 0.0, 1.0)

        reliability = 0.55 * item_factor + 0.30 * concentration_factor + 0.15 * discrimination_factor
        return float(np.clip(reliability, 0.15, 0.95))

    @staticmethod
    def shrink_theta(theta: float, reliability: float) -> float:
        """向中性偏好收縮，避免用低品質問卷把人過度分到極端風險型。"""
        return float(0.5 + (theta - 0.5) * reliability)
...
questionnaire_reliability = PsychologicalFitGuard.estimate_reliability(
    response_vector,
    reliability_weights,
)

# 反對觀點：側欄題數有限，先將極端風險偏好收斂至中性
theta = PsychologicalFitGuard.shrink_theta(theta, questionnaire_reliability)
...
risk_profile = RiskProfile(
    ...
    questionnaire_reliability=questionnaire_reliability
)
...
class FinancialAnalyzer:
    ...
    def calculate_utility_score(metrics: FinancialMetrics,
        ...
        # 問卷可靠度不足時，降低效用分數幅度，避免把「心理適配」過度解讀為精確排序
        reliability = np.clip(risk_profile.questionnaire_reliability, 0.15, 0.95)
        final_utility = final_utility * reliability
        cer = cer * reliability
